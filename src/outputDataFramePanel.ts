...
import { getSharedEnrichedSchema, outputColumnDocTooltip } from './enrichedSchema';
...
export class SwatOutputDataFramePanel {
    ...
    private getHtml(parsed: ParsedOutputDataFrame): string {
        ...
        const fileDoc = getSharedEnrichedSchema()?.getOutputFileDoc(parsed.fileName);
        const subText = fileDoc?.summary
            ? escapeHtml(fileDoc.summary)
            : 'Search, sort, filter, and chart output files directly in VS Code.';
...
${this.getScript(parsed)}
    ...
    private getScript(parsed: ParsedOutputDataFrame): string {
        ...
        // Per-column plain-text documentation tooltips, aligned with columns.
        const enriched = getSharedEnrichedSchema();
        const columnDocs = parsed.columns.map(
            column => outputColumnDocTooltip(enriched?.getOutputColumnDoc(parsed.fileName, column)));
        return [
            `const data = ${data};`,
            `const columnDocs = ${JSON.stringify(columnDocs)};`,
            ...
            'function renderTable(visibleRows) { const headers = columns.map((column, index) => { const glyph = state.sortColumn === column ? (state.sortDirection === "asc" ? "ASC" : "DESC") : "SORT"; const unit = Array.isArray(data.units) ? String(data.units[index] ?? "").trim() : ""; const unitMarkup = unit ? `<span class="headUnit">${escapeHtml(unit)}</span>` : ""; const doc = Array.isArray(columnDocs) ? String(columnDocs[index] ?? "").trim() : ""; const titleAttr = doc ? ` title="${escapeHtml(doc)}"` : ""; return `<th${titleAttr}><button type="button" class="sortBtn" data-column="${escapeHtml(column)}"><span class="headText"><span class="headTitle">${escapeHtml(column)}</span>${unitMarkup}</span><span class="glyph">${glyph}</span></button></th>`; }).join(""); el.tableHead.innerHTML = `<tr><th>#</th>${headers}</tr>`; if (!visibleRows.length) { el.tableBody.innerHTML = ""; el.emptyState.hidden = false; return; } el.emptyState.hidden = true; el.tableBody.innerHTML = visibleRows.map(row => `<tr><td class="rowIndex">${row.previewRow}</td>${row.values.map(value => `<td>${escapeHtml(String(value ?? ""))}</td>`).join("")}</tr>`).join(""); }',
